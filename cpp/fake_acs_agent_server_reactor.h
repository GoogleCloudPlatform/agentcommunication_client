...
class FakeAcsAgentServerReactor
 ...
 private:
  ...
  void OnReadDone(bool ok) ABSL_LOCKS_EXCLUDED(response_mtx_) override {
    if (!ok) {
      Finish(
          grpc::Status(grpc::StatusCode::UNKNOWN, "Unexpected read failure"));
      return;
    }
  ...
  void OnWriteDone(bool ok) ABSL_LOCKS_EXCLUDED(response_mtx_) override {
    if (!ok) {
      Finish(
          grpc::Status(grpc::StatusCode::UNKNOWN, "Unexpected write failure"));
      return;
    }
    absl::MutexLock lock(&response_mtx_);
    writing_ = false;
...
class FakeAcsAgentServer {
 public:
  FakeAcsAgentServer(grpc::Service* service) {
