...
absl::StatusOr<AcsToken> GetAndParseAcsToken(absl::string_view endpoint) {
...
absl::StatusOr<AgentConnectionId> GenerateAgentConnectionId(
  ...
  absl::StatusOr<AcsToken> AcsToken = GetAndParseAcsToken(kAcsTokenEndpointGce);
  if (!AcsToken.ok()) {
    // If the token is not available from the GCE endpoint, try the GKE
    // endpoint.
    AcsToken = GetAndParseAcsToken(kAcsTokenEndpointGke);
    if (!AcsToken.ok()) {
