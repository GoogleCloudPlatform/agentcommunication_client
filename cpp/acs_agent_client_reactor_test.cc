...
TEST_F(AcsAgentClientReactorTest, TestAddRequest) {
  ...
  std::unique_ptr<Request> request =
      agent_communication::MakeRequestWithBody("body test");
  std::unique_ptr<Request> request_copy = std::make_unique<Request>(*request);

  // Add the request to the reactor.
  EXPECT_TRUE(reactor_->AddRequest(request));
...
TEST_F(AcsAgentClientReactorTest, TestOnReadSuccessfully) {
  ...
  std::unique_ptr<Response> response_copy =
  ...
  EXPECT_EQ(response_status, std::future_status::ready);
