...
TEST_F(AcsAgentClientReactorTest, TestReactorCanInitializeQuota) {
  ...
  ASSERT_TRUE(WaitUntil(
      ...
      absl::Seconds(10)));
  EXPECT_EQ(*reactor_->GetMessagesPerMinuteQuota(), 1);
  EXPECT_EQ(*reactor_->GetBytesPerMinuteQuota(), 100);
}
...
TEST_F(AcsAgentClientReactorTest,
  ...
  EXPECT_EQ(reactor_->GetMessagesPerMinuteQuota().status().code(),
            absl::StatusCode::kFailedPrecondition);
  EXPECT_EQ(reactor_->GetBytesPerMinuteQuota().status().code(),
            absl::StatusCode::kFailedPrecondition);
  ...
  ASSERT_TRUE(WaitUntil(
      ...
      absl::Seconds(10)));
  EXPECT_THAT(*reactor_->GetMessagesPerMinuteQuota(), AnyOf(1, 100));
}
