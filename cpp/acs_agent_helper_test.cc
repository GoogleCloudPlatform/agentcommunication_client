...
TEST(AcsAgentHelperTest, MakeRequestWithBody) {
  std::string message = "message";
  std::unique_ptr<Request> request = MakeRequestWithBody(message);
  EXPECT_EQ(request->message_body().body().value(), message);
  EXPECT_EQ(request->message_id().empty(), false);
}
...
TEST(AcsAgentHelperTest, MakeResponseWithBody) {
  std::string message = "message";
  std::unique_ptr<Response> response = MakeResponseWithBody(message);
  EXPECT_EQ(response->message_body().body().value(), message);
  EXPECT_EQ(response->message_id().empty(), false);
}
